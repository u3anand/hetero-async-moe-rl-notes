...
| [[GEM]] | 2605.19945 | **GPU-variability-aware expert→GPU mapping**, same straggler motivation, "consistent vs temporal" experts | **static/profiled** (mapped once at load, *names* "no online adaptation to distribution shift" as its limit); **intra-generation** variability (7.7–27.7% among identical L40s), not a 2–4× tier gap; **no replication/token-split** (one expert per GPU); latency-only |
| [[HarMoEny]] | 2506.12417 | **online** token redistribution + async expert prefetch vs load imbalance | **homogeneous** (redistributes to *under-utilized*, not *faster*, GPUs — no tier-speed model); throughput/TTFT objective, not p99/$ |
| [[HexAGenT]] | 2605.16637 | hetero PD **routing**, agentic | dense; routes over fixed pools, no expert placement |

**The sharpened claim (the novelty fence — say exactly this, not "nobody has done hetero expert mapping"):** the literature splits three ways, and each gives up a different axis — (i) **static/profiled** heterogeneity-aware *mapping* ([[GEM]], [[Aurora]], [[MegaScale-Infer]]) — mapped once, no online adaptation, one-expert-one-GPU; (ii) **homogeneous** online replication/prefetch/redistribution ([[HarMoEny]], CRAFT, PROBE) — adapts online but assumes equal-speed GPUs, so "which tier" is degenerate; (iii) **memory-tier** expert caching ([[Toward-Efficient-MoE-Inference]], [[SwapMoE]]) — GPU↔CPU stall-on-miss, not compute-tier load-balancing. **TierShift is the one combination none hold: online tier-aware placement + migration + replicate-and-split across a real fast/slow compute gap, under a p99-SLO + $/token objective, with an explicit migration-timescale regime map.** [[GEM]] is the closest and even *names* "no online adaptation to distribution shift" as its own limitation — that limitation is our wedge.

> **TierShift vs. GEM (the dart we must parry first):** GEM exploits **~10–25% manufacturing variability among *identical* GPUs** with a **one-time static** map and **one expert per GPU**; TierShift exploits a **2.4–4× *architectural* gap** with **online** migration and **replicate-and-split**. The magnitude is the crux: at GEM's 7.7–27.7%, a misplaced hot expert costs ~10–25% → a static skew suffices (*why GEM is static*); at 2.4–4×, it's a 240–400% penalty → online correction pays for itself, and a single expert hotter than one GPU *must* be split across tiers (which GEM structurally cannot do). So "online" and "split" aren't GEM+2 add-ons — they're **forced by the regime GEM explicitly excludes**. The proof obligation: show **static-GEM-style violates the SLO under drift** and **replicate-and-split beats it**, with the win traced to **temporal experts** (GEM handles *consistent* ones statically). If that margin is ~0, the honest answer is "static suffices here" — hence B2's headline is the **regime map**, not "TierShift wins."
...
Before building a controller, measure **how expert hotness actually moves** on a real serving stream, on real heterogeneous tiers. **B2's headline is a *regime identification*, not "TierShift works":**

> **Which regime does heterogeneous MoE serving live in — *static-sufficient*, *replication-friendly*, or *migration-friendly*?** — as a function of (hotness drift × tier gap × migration cost × VRAM budget).

Framed this way, B2 is **publishable whatever the answer** (a negative — "static suffices for this model/workload" — is a result, and it's the scientist's framing, not a controller-salesman's). The charts *are* the contribution. Two questions decide the regime: (1) does hotness non-stationarity interact with the tier gap strongly enough to defeat static placement; (2) the **kill-question** — *does hotness shift slower than an expert can migrate over PCIe?* If experts churn faster than PCIe can move them, the regime is replication-only (or static), not migration.
...
Reported as **p50/p95/p99** over steady-state windows. **Success = the regime is identified with evidence**, whichever it is: *migration-friendly* (Q1/Q2 show drift defeats static **and** Q3 shows migration ≪ churn), *replication-friendly* (drift defeats static but Q3 fails → replicate-and-split without migration), or *static-sufficient* (drift too weak / routing too uniform → static mapping à la [[GEM]] wins, a clean negative). Each is a publishable characterization; only the first justifies the full migrating controller.
...
**Why this isn't the retired ideas:** not generic expert load-balancing (homogeneous EPLB/CRAFT — equal devices, no tier asymmetry); not **static** hetero mapping ([[GEM]] maps once and names "no online adaptation" as its limit; Aurora proved the static hard case); not **homogeneous** dynamic redistribution ([[HarMoEny]] moves tokens to *idle* GPUs, not *faster* ones — no tier-speed split); not attention/expert tier-splitting (HeterMoE/MegaScale own it); **and not just Expert Buffering** (memory-capacity caching, stall-on-miss, GPU↔CPU). TierShift = **online**, **compute-tier** (not memory-tier), **speed-aware token-split**, under **p99/$**.
...
**Evaluation:** two framings of the same runs — *(i) fixed fleet → SLO-attainment + throughput; (ii) fixed SLO → $/token (min-cost fleet that still meets p99).* Baselines (named so reviewers can't ask "what about X"):
- **(a) static-avg-opt** — offline optimum expert→tier mapping for the *average* trace (Aurora-style).
- **(b) GEM-style** — static **variability/speed-aware** mapping (approximated): speed-proportional placement, one-expert-one-GPU, **no online adaptation**. *The closest static hetero baseline.*
- **(c) naive cache-port** — reactive LRU hot→fast, no split (the Layer-1 "just copy Expert Buffering" baseline — *the make-or-break one to beat*).
- **(d) HarMoEny/PROBE-style** — **homogeneous** dynamic token redistribution / prefetch (approximated): rebalance to under-utilized GPUs *without* a tier-speed model.
- **(e) CRAFT-style** — cost-aware replication, blind to tiers.
- **(f) round-robin EP**; **(g) all-fast-GPU** — the expensive SLO-meeting reference for the cost story.

Ablate baseline → +M1 (reactive cache) → +M2 (replicate-split) → +M3 (tier-gap policy). **Success criteria:** (1) **$/token at fixed p99 SLO** below all-fast (g); (2) **p99 SLO held under drift** where static (a)/(b) violate it; (3) **make-or-break:** headroom of (M1+M2+M3) over (c), *and* over the closest dynamic neighbor (d). Supporting metrics: throughput, per-tier busy%, slow-tier straggler wait, migration bytes/s, imbalance ratio over time.
